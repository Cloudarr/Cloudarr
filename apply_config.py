...
if MANAGE_TRAEFIK_DYNAMIC: 
  ...
  for prefix, conf in loaded_services.items():
    ...
    tls_version = conf.get("tls", "intermediate")
    insecure = not route.lower().startswith("http")
    router = {
      "entrypoints":["https"],
      "rule": f'Host(`{prefix}.{domainname}`)',
      "service": prefix,
      "tls": {"options": f"{tls_version}@file"}
    }
...
"""
  file_path = os.path.join(config_dir, "guacamole/postgress/init/initdb.sql")
