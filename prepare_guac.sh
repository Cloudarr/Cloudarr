...
echo "Preparing folder init and creating $CONFIGDIR/guacamole/postgress/init/initdb.sql"
mkdir  -p $CONFIGDIR/guacamole/postgress/init/ >/dev/null 2>&1
chmod -R +x $CONFIGDIR/guacamole/postgress/init/
docker run --rm guacamole/guacamole /opt/guacamole/bin/initdb.sh --postgresql > $CONFIGDIR/guacamole/postgress/init/initdb.sql
echo "done"
